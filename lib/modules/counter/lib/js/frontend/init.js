window.addEventListener('load', function() {
	let success_layer			= document.createElement('div');
	...
	function sv_wishlist_refresh_counter(method, key, args){
		...
		document.querySelectorAll('.wp-block-group.is-style-wishlist-counter > i.count').forEach(counter => {
			...
			if(counter_ajax){
				counter_ajax.classList.remove('active');
			}*/
		});
	}
	...
	document.querySelectorAll('.is-style-wishlist-counter').forEach(element => {
		...
		element.addEventListener('click', function (e) {
			...
			if(svwfw.get_items_count() === 0){
				return;
			}
			
			document.getElementById('sv_wishlist_for_woocommerce_counter_ajax').classList.add('active');
			...
			.then(function (r) {
				...
				document.getElementById('sv_wishlist_for_woocommerce_counter_ajax').innerHTML 	= r.msg;
				...
				document.querySelectorAll('.sv_wishlist_for_woocommerce_counter_remove').forEach(element => {
					...
					document.querySelector('.sv_wishlist_for_woocommerce_counter_close').addEventListener('click', function (e) {
						document.getElementById('sv_wishlist_for_woocommerce_counter_ajax').classList.remove('active');
					});
				});
			}).catch(r => {
				// error
